      ...
      if (path && path.length == 3) {
        this.$http({
          url: this.$http.adornUrl(
            `/product/attr-groups/${path[path.length - 1]}`
          ),
          method: "get",
  ...
  methods: {
    init (id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.attrId) {
          this.$http({
            url: this.$http.adornUrl(
              `/product/attr/${this.dataForm.attrId}`
            ),
            method: "get",
    ...
    dataFormSubmit () {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          let uri = !this.dataForm.attrId ? '/product/attrs' : '/product/attr/' + this.dataForm.attrId
          let method = !this.dataForm.attrId ? 'post' : 'put'
          this.$http({
